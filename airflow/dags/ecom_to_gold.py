...
check_data_in_silver_ecom = SSHOperator(
    ...
    command="""
    set -euo pipefail
    path=/data/silver/ecom
    if ! /usr/local/hadoop/bin/hdfs dfs -test -d "$path"; then
        echo "Missing HDFS path: $path"
        exit 1
    fi
    file_count=$(/usr/local/hadoop/bin/hdfs dfs -count "$path" | awk '{print $2}')
