...
- **Ingestion**: Structured data from `PostgreSQL` databases (Ecom and Marketing) is ingested into the **Bronze layer** of the *Data Lakehouse* using `Spark JDBC`, scheduled through `Apache Airflow` for regular updates. This ensures that fresh data is continuously available for processing.
